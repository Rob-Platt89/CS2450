...
EMPLOYEE_CSV = dir_path + slash_direction + 'employees.csv'
...
RECEIPTS_CSV = dir_path + slash_direction + 'receipts.csv'
TIMECARDS_CSV = dir_path + slash_direction + 'timecards.csv'
...
class EMPLOYEE_SEARCH_FRAME(ttk.Frame):
    ...
    def update_frame(self):
        ...
        if self.controller.user_permission_level == '1':
            ...
            try:
                self.receipt_button.destroy()
                self.timecard_button.destroy()
            except:
                print('error')
        ...
        if self.controller.user_permission_level == '1':
            ...
            try:
                self.archive_button.destroy()
                self.run_payroll_button.destroy()
                self.unarchive_button.destroy()
            except:
                print('error')
    ...
    def add_receipt(self, employee):
        if employee == None: # No employee was selected
            return
    ...
    def view_employee_base(self, employee):
        if employee == None: # no employee was selected when function was called
            return
...
class LOGIN_FRAME(ttk.Frame):
    ...
    def failed_login(self, message):
        ...
        self.error_label.grid(column=0, columnspan=3, row=5, sticky='W')
